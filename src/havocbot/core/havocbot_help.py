...
class HelpPlugin(HavocBotPlugin):
    ...
    def get_plugin_help_matching_list(self, tuples_list):
        ...
        for (plugin_class, dictionary) in tuples_list:
            if dictionary is not None:
                if 'description' in dictionary and 'short_name' in dictionary:
                    new_list.append("%s (%s) - %s" % (plugin_class, dictionary['short_name'], dictionary['description']))
                if 'usage' in dictionary:
                    for item in self.get_usage_lines_from_plugin_as_list(dictionary['usage']):
                        new_list.append(item)
            else:
                new_list.append("%s" % (plugin_class))
