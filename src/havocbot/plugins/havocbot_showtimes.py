...
class ShowtimesPlugin(HavocBotPlugin):
    ...
    def plugin_usages(self):
        return [
            Usage(command='movies in <zip code>', example='movies in 94110',
                  description='displays the earliest upcoming movie showtimes in a zip code (AMC only)'),
        ]
    ...
    def configure(self, settings):
        ...
        if self.api_key_amc is not None and len(self.api_key_amc) > 0:
            if self.max_distance_in_miles is not None and isinstance(self.max_distance_in_miles, int):
                if self.max_upcoming_showtimes_to_display is not None and isinstance(
                        self.max_upcoming_showtimes_to_display, int):
                    requirements_met = True
                else:
                    logger.error('There was an issue with the max_upcoming_showtimes_to_display setting. '
                                 'Verify that the max_upcoming_showtimes_to_display key is set in the settings file')
            else:
                logger.error('There was an issue with the max_distance_in_miles setting. '
                             'Verify that the max_distance_in_miles key is set in the settings file')
        else:
            logger.error('There was an issue with the api key. '
                         'Verify that the api_key_amc key is set in the settings file')
    ...
    def trigger_default(self, client, message, **kwargs):
        ...
        logger.debug('searching for movies for %s' % zip_code)
        showtimes_object = showtimes.get_showtimes_for_zip_on_date(
            zip_code, datetime.now().strftime('%m-%d-%Y'), self.api_key_amc,
            self.max_distance_in_miles, self.max_upcoming_showtimes_to_display)
