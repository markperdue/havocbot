...
class WeatherPlugin(HavocBotPlugin):
    ...
    def plugin_usages(self):
        return [
            Usage(command='weather <zip code>', example='what is the weather in 94110',
                  description='display the current weather for one or more zip codes'),
            Usage(command='warmest weather <zip code> <zip code> [<zip code>]',
                  example='does 92104 or 10005 have the warmest weather',
                  description='find the warmest zip code from a list'),
        ]

    @property
    def plugin_triggers(self):
        return [
            Trigger(match='.*\d{5}.*weather|weather.*\d{5}', function=self.trigger_default, requires=None),
        ]
    ...
    def configure(self, settings):
        ...
        if (self.api_key_weatherunderground is not None and len(self.api_key_weatherunderground) > 0) or \
                (self.api_key_openweathermap is not None and len(self.api_key_openweathermap) > 0):
            requirements_met = True
        else:
            logger.error('There was an issue with the api key. '
                         'Verify either an api_key_weatherunderground key is set or '
                         'an api_key_openweathermap key is set in the settings file')
                ...
                if message.to:
                    if warmest_weather:
                        text = '%s (%s) has the warmest weather of %sF' % (
                            warmest_weather.city, warmest_weather.zip_code, warmest_weather.temperature)
